...
def _save_output(image: Image.Image) -> Path:
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
    output_path = OUTPUT_DIR / f"roommorph_{timestamp}_{uuid.uuid4().hex[:8]}.png"
    ...
    with gr.Blocks(**blocks_kwargs) as demo:
        with gr.Column(elem_id="app-shell", min_width=0):
            with gr.Column(elem_id="app-header", min_width=0):
                gr.Markdown(
                    "# RoomMorph AI\n"
                    "Upload a room photo and turn it into an interior-design concept using "
                    "this repository's custom Stable Diffusion 1.5 image-to-image pipeline."
                )
                gr.Markdown(
                    f"**Runtime device:** `{RUN_DEVICE.type}`  \n"
